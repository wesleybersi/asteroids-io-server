...
export class Inventory {
  ...
  hotkeys: (InventoryItem | null)[] = [null, null, null, null];
  ...
  constructor(player: Player, rows: number, cols: number) {
    ...
    // this.player.primaryWeapon = createNewWeapon(
    //   this.player,
    //   this.hotkeys[0]?.key ?? "",
    //   this.hotkeys[0]?.durability ?? 100
    // );
    // this.player.secondaryWeapon = createNewWeapon(
    //   this.player,
    //   this.hotkeys[1]?.key ?? "",
    //   this.hotkeys[1]?.durability ?? 100
    // );
  }
  ...
  deselectHotKey() {
    if (this.player.primaryWeapon instanceof Crossbow) {
      this.player.primaryWeapon.unload();
    } else if (this.player.primaryWeapon instanceof Bow) {
      if (this.player.primaryWeapon.heldProjectile) {
        this.player.primaryWeapon.heldProjectile.state = "removed";
        this.player.primaryWeapon.heldProjectile = null;
        // this.player.projectiles.arrows++;
        //TODO
      }
    }
    const key = this.hotkeys[this.hotkeyIndex];
    if (key && this.player.primaryWeapon) {
      key.durability = this.player.primaryWeapon?.durability.current;
    }
    this.hotkeyIndex = -1;
    // this.player.primaryWeapon = null;
    return;
  }
