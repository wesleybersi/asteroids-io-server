...
export class Bow extends Weapon {
  ...
  maxSpread = 6;
  overload = { current: 0, multiplier: 1, isOverloaded: false };
  ...
  update(delta: number) {
    ...
    if (this.player.isPointerDown.left) {
      ...
      if (this.heldProjectile) {
        this.release();
        this.overload.current += delta * this.overload.multiplier * 10;
  ...
  hold(delta: number) {
    ...
    // this.force.current += delta * Infinity;
  ...
  release() {
    const { player } = this;
    const offset = 64 - this.force.current * 70;
    const { x, y } = getAngleOffset(player.x, player.y, player.angle, offset);

    if (this.force.current < 1) {
      if (this.heldProjectile) {
